...
class DtoClassWriterTest extends TestCase
{
    ...
    public function testWriteCreatesDirectoryAndFile(): void
    {
        ...
        self::assertFalse($this->root->hasChild('Generated/Dto/SomeEntity'));

        // Execute the write method
        $returnedPath = $this->writer->write($this->context, $className, $code);

        // Assertions
        self::assertEquals($expectedPath, $returnedPath);

        // Check directory exists
        self::assertTrue($this->root->hasChild('Generated/Dto/SomeEntity'), 'Directory Generated/Dto/SomeEntity should have been created.');

        // Check file exists within the directory
        $dtoDir = $this->root->getChild('Generated/Dto/SomeEntity');
        self::assertInstanceOf(vfsStreamDirectory::class, $dtoDir); // Make sure it's a directory
        self::assertTrue($dtoDir->hasChild($className . '.php'), "File {$className}.php should exist in the created directory.");

        // Check file content
        self::assertEquals($code, $dtoDir->getChild($className . '.php')->getContent());
    }
    ...
    public function testWriteInvalidCodeThrowsExceptionAndDoesNotWrite(): void
    {
        ...
        try {
            ...
            self::assertInstanceOf(ParserError::class, $e->getPrevious());
